...
- Frontend runtime: `apps/web` is a Next.js app targeting Vercel. The public recruiter surface uses bundled snapshots with a tokenless preview identity, except for the live bounded Discover page. Supabase browser session restoration remains only for an already-valid non-demo operator session. Discovery-admin leads/search-task reads now go through the Next admin proxy, but some other operational reads still go browser-direct on the private live surface.
- Recruiter demo API runtime: the Vercel deployment points `NEXT_PUBLIC_API_BASE_URL` and `API_BASE_URL` at the Supabase Edge Function API. The public Discover page presents four deliberately distinct ICP markets—multi-location dental groups, boutique hospitality, commercial solar/roofing contractors, and B2B developer platforms—and defaults to United States targeting across New York, Los Angeles, Chicago, Houston, and Phoenix. It uses an HttpOnly browser-session cookie and an exact same-origin proxy allowlist for curated ICPs/settings, same-session run history and performance drill-downs, and fixed five-search-task SerpAPI runs. Performance responses expose run timing and provider-task counts without lead/contact data and remain scoped to the creating browser session. One SQL transaction atomically reserves provider-search credits, creates the durable run, and enqueues bounded `discovery.seed` shards for the production worker pipeline. Stale queued/running demo runs are reconciled to a truthful terminal failure on the next session read/create. Reserved-task caps are 25 per browser session/day, 2 concurrent runs, and 50 globally/day. Normal Edge routes remain private, and outbound delivery remains disabled.
...
- Worker runtime: `apps/worker` remains the server-only background execution path for the full service and now executes the bounded public Discover pipeline through durable `discovery.seed` jobs. Messaging, outbound, and unrelated worker-backed actions stay disabled in the demo.
