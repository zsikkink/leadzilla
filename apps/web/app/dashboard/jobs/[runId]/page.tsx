...
function LiveDiscoveryRunDetailPage({ runId }: { runId: string }) {
        ...
        try {
          ...
          if (status !== null && status >= 500) {
            ...
            setCancelError(
              toSafeDisplayErrorMessage(
                retryError,
                'Cancellation is still processing. Refresh the run and try again in a moment.',
              ),
            );
          }
  ...
  const runErrorMessage = toDiscoveryRunNotice(run.data?.errorMessage);
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="rounded-2xl border border-border/50 bg-card p-8 text-center shadow-sm">
          ...
          <p className="text-sm font-bold text-foreground/80">Run summary is refreshing</p>
          <p className="mt-1 text-[11px] text-muted-foreground/50">
            Return to Discover and retry this run in a moment.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      ...
      {details.error && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/5 px-4 py-2.5">
          <p className="text-[11px] text-amber-200/80">
            Run details are refreshing. The summary above remains available.
          </p>
        </div>
      )}
      ...
      {/* Incomplete task notes (collapsed by default) */}
      {failedTasks.length > 0 && (
        <div className="rounded-2xl border border-amber-500/20 bg-card p-5 shadow-sm">
          <button
            type="button"
          ...
          >
            ...
            <span className="rounded-md bg-amber-500/10 px-2 py-0.5 font-mono text-[11px] font-bold tabular-nums text-amber-300">
              {failedTasks.length}
            </span>
            <ChevronDown
          ...
          {errorsExpanded && (
            <div className="mt-3 space-y-2">
              {failedTasks.map((task) => (
                <div
                  key={task.id}
                  className="rounded-lg bg-amber-500/5 px-3 py-2"
                >
                  <p className="text-xs font-semibold text-foreground/80">
                    {task.queryText}
                  </p>
                  <p className="mt-0.5 text-[11px] text-amber-200/75">
                    {toDiscoveryRunNotice(task.error) ?? 'This search task could not be completed.'}
                  </p>
                </div>
              ))}
            </div>
      ...
      {runErrorMessage && (
        <div className="rounded-xl border border-amber-500/30 bg-amber-500/5 px-4 py-3">
          <p className="text-xs font-bold text-amber-300">Run Note</p>
