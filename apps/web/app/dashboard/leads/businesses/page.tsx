  ...
  const fetchBusinesses = useCallback(async (pageNum: number, search: string) => {
    ...
    try {
      ...
      if (rows.length > 0) {
        ...
        // BusinessConversion has NO @map — use camelCase column names
        ...
        if (conversions && conversions.length > 0) {
          for (const conv of conversions as Array<{ businessId: string; leadId: string }>) {
            convertedBizIds.add(conv.businessId);
          }

          const leadIds = [...new Set(conversions.map((c: { leadId: string }) => c.leadId))];
          ...
          if (leads) {
            ...
            for (const conv of conversions) {
              const c = conv as { businessId: string; leadId: string };
              if (!bizLeadMap.has(c.businessId)) {
                bizLeadMap.set(c.businessId, c.leadId);
              }
            }
  ...
  useEffect(() => {
    ...
    async function fetchContacts() {
      try {
        ...
        if (contactsData && contactsData.length > 0 && !cancelled) {
          setSelectedContacts(contactsData.map((c: { id: string; name: string; title: string | null; email: string | null; linkedinUrl: string | null; source: string | null }) => ({
            id: c.id,
            fullName: c.name,
            jobTitle: c.title,
            email: c.email,
