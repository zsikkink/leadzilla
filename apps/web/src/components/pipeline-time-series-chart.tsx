...
// ── Supabase fetch with ICP filtering ──────────────────────────────────────
...
export function PipelineTimeSeriesChart({ icpProfileId }: PipelineTimeSeriesChartProps) {
  ...
  // Fetch leads from Supabase directly with optional ICP filtering
  useEffect(() => {
    let cancelled = false;
    setIsLoading(true);
    setError(null);

    fetchLeadRows(range, icpProfileId)
      .then((data) => {
        if (!cancelled) {
          setRows(data);
          setIsLoading(false);
      ...
      <div className="relative z-10 mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-white/40">
            Daily lead flow across pipeline stages{icpProfileId ? ' (filtered by ICP)' : ''}
          </p>
        </div>
